    ...
    return (
      <>
        ...
        <div className="min-h-[calc(100vh-200px)] flex flex-col items-center justify-center text-center bg-cream-bg py-12 px-4">
          ...
          <h1 className="text-4xl font-playfair font-bold text-deep-brown mb-4">Your Cart is Empty</h1>
          <p className="text-xl text-deep-brown/70 mb-8">Looks like you haven't added anything to your cart yet.</p>
        ...
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="bg-white shadow-xl rounded-2xl p-6 md:p-8">
            {items.map((item: CartItem) => (
              <div key={item.id} className="flex flex-col md:flex-row items-center py-6 border-b border-warm-gold/20 last:border-b-0">
                <img 
                ...
                <div className="flex-grow text-center md:text-left">
                  <h2 className="text-xl font-playfair font-semibold text-deep-brown">{item.name}</h2>
                  <p className="text-md text-deep-brown/80">Size: {item.variant.size}</p>
                  <p className="text-lg font-semibold text-warm-gold mt-1">₹{item.price.toFixed(2)}</p>
                </div>
                ...
                <div className="w-full md:w-auto md:text-right mt-2 md:mt-0 md:ml-6">
                  <p className="text-lg font-playfair font-bold text-deep-brown">₹{(item.price * item.quantity).toFixed(2)}</p>
                </div>
              </div>
            ))}
            ...
            <div className="mt-8 pt-6 border-t border-warm-gold/30">
              <div className="flex justify-between items-center mb-2">
                ...
                <p className="text-2xl font-playfair font-bold text-warm-gold">₹{getTotalPrice().toFixed(2)}</p>
              </div>
              <p className="text-sm text-deep-brown/60 text-right mb-6">Shipping and taxes calculated at checkout.</p>
              <div className="flex flex-col md:flex-row justify-between items-center space-y-3 md:space-y-0 md:space-x-4">
                <Button asChild variant="outline" className="w-full md:w-auto border-warm-gold text-warm-gold hover:bg-warm-gold/10 hover:text-rich-brown">
                  <Link href="/">
                    <ArrowLeft className="w-4 h-4 mr-2" /> Continue Shopping
                  </Link>
                </Button>
